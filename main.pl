...
take :- (koordinatpemain(A,B), ground(A,B,X)) -> assertz(pemain(X)).
%inventoripemainbego2an([X|Y]) :- pemain([X|Y]), write(X), nl, inventoripemainbego2an([Y]).
%inventoripemainbego2an([])  :- write('').
%inventoripemainbego2an(X) :- pemain(X), write(''

inventoripemain :- pemain(X), write(X).
